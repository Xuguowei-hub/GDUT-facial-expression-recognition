...
model_path = '/media/6t/XGW/Conv/face_exp/save/'
...
model = ResNet(BasicBlock, [18,18,18], num_classes = 3).to(device=f'cuda:{cuda}')
...
#model.load_state_dict(torch.load(model_path + 'resnet/' + "resnet18/0:0.509.pt"))
model.eval()
count = 0
pred_all = np.empty(0)
...
with torch.no_grad():
    for data, label in valid_loader:
        ...
        if count == 0:
            ...
            label_all = label.detach().cpu().numpy()
            count += 1
        else:
            pred_all = np.vstack((pred_all, pred.detach().cpu().numpy()))
            label_all = np.hstack((label_all, label.detach().cpu().numpy()))
#np.save(model_path + 'vit/'+"weight_nonaug_vit_pred.npy",pred_all)
#np.save(model_path + 'vit/'+"weight_nonaug_vit_label.npy",label_all)
